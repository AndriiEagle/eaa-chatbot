  ...
  try {
    ...
    if (checkError) {
      if (checkError.message.includes('updated_at does not exist')) {
        console.log(
          'Обнаружена проблема: колонка updated_at отсутствует в таблице user_facts'
        );
        ...
        if (alterError) {
          console.error(
            '❌ Ошибка при добавлении колонки updated_at:',
            alterError
          );
          ...
          try {
            ...
            console.log(
              '✅ Колонка updated_at успешно добавлена через add_column_if_not_exists'
            );
          } catch (rpcError) {
            console.error(
              '❌ Ошибка при использовании add_column_if_not_exists:',
              rpcError
            );
        ...
        if (updateError) {
          console.error(
            '❌ Ошибка при обновлении значений updated_at:',
            updateError
          );
        } else {
        ...
        console.error(
          '❌ Ошибка при проверке схемы таблицы user_facts:',
          checkError
        );
      }
    } else {
      ...
      try {
        ...
        console.error(
          '❌ Исключение при прямом обновлении схемы:',
          directUpdateError
        );
      }
    }
    ...
    if (finalError) {
      ...
      console.log(
        '✅ Итоговая проверка успешна. Схема таблицы user_facts обновлена.'
      );
    }
  } catch (e) {
