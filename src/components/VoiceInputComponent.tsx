...
const VoiceInputComponent: React.FC<VoiceInputProps> = ({
  ...
  const [settings, setSettings] = useState<VoiceSettings>({
    ...
    customWakeWords: ['hello assistant', 'привет ассистент'],
  ...
  useEffect(() => {
    console.log('[VoiceInput] Attempting to initialize processor...');
    // Initialize Whisper processor when component mounts
    // Инициализируем Whisper процессор при монтировании компонента
    if (!processorRef.current) {
      try {
        const openaiClient = new OpenAI({
          apiKey: process.env.REACT_APP_OPENAI_API_KEY || "YOUR_DUMMY_API_KEY", // Используйте переменную окружения
        ...
        console.log('[VoiceInput] Whisper processor successfully initialized.');
      } catch (error) {
        console.error('[VoiceInput] FAILED to initialize Whisper processor:', error);
        onError?.('Failed to initialize speech recognition service.');
      }
    }
  }, [onError]);
  ...
  const startRecording = useCallback(async () => {
    console.log('[VoiceInput] startRecording: Starting recording...');
    try {
      ...
      console.log('[VoiceInput] startRecording: MediaRecorder started.');
      ...
      if (enableRealTime && processorRef.current) {
        console.log('[VoiceInput] startRecording: Starting real-time processing.');
      ...
      console.error('[VoiceInput] startRecording: Error starting recording:', error);
    }
  }, [language, enableRealTime, noiseReduction, voiceActivation, audioQuality, onError]);
  ...
  const stopRecording = useCallback(async () => {
    console.log('[VoiceInput] stopRecording: Stopping recording...');
    if (mediaRecorderRef.current && recordingState.isRecording) {
      mediaRecorderRef.current.stop();
      console.log('[VoiceInput] stopRecording: MediaRecorder.stop() called.');
  ...
  const handleAudioData = (event: BlobEvent) => {
    if (event.data.size > 0) {
      console.log(`[VoiceInput] handleAudioData: Received audio chunk, size: ${event.data.size} bytes`);
  ...
  const handleRecordingStop = async () => {
    console.log(`[VoiceInput] handleRecordingStop: Recording stopped. Audio chunks: ${audioChunksRef.current.length}`);
    if (audioChunksRef.current.length === 0) {
      console.warn('[VoiceInput] handleRecordingStop: No audio data to process.');
      return;
    }
    
    const audioBlob = new Blob(audioChunksRef.current, { type: 'audio/webm' });
    console.log(`[VoiceInput] handleRecordingStop: Created Blob, size: ${audioBlob.size} bytes`);
    ...
    try {
      if (processorRef.current) {
        console.log('[VoiceInput] handleRecordingStop: Calling processor.processLongAudio...');
        ...
        console.log('[VoiceInput] handleRecordingStop: Processing completed. Result:', result);
        ...
        console.error('[VoiceInput] handleRecordingStop: processorRef.current does NOT exist! Processing impossible.');
        onError?.('Error: Speech recognition service not initialized.');
      }
    } catch (error) {
      console.error('[VoiceInput] handleRecordingStop: Error processing audio:', error);
  ...
  if (compact) {
    return (
      <button
        className="voice-record-btn-compact"
        ...
        style={{
          ...
          backgroundColor: recordingState.isRecording ? '#dc2626' : '#374151',
          border: 'none',
          cursor: 'pointer',
        }}
        title={recordingState.isRecording ? 'Stop recording' : 'Voice input'}
      >
        {processingState.isProcessing ? (
          <RotateCcw size={18} color="#ffffff" style={{ animation: 'spin 1s linear infinite' }} />
        ) : recordingState.isRecording ? (
  ...
  return (
    <div className={`voice-input-component ${theme} ${disabled ? 'disabled' : ''}`}>
      ...
      <div className="voice-main-interface">
        ...
        >
          <div className="voice-btn-icon" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {processingState.isProcessing ? (
              <RotateCcw size={20} color="#ffffff" style={{ animation: 'spin 1s linear infinite' }} />
            ) : recordingState.isRecording ? (
