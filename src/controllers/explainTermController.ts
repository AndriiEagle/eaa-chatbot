 ...
 * - "Что означает WCAG?"
 */
export const explainTermController = async (
  req: Request,
  res: Response
): Promise<void> => {
  ...
  try {
    ...
    console.log(
      `👤 [EXPLAIN_TERM] User: ${user_id} | Session: ${session_id || 'new'}`
    );

    if (context) {
      console.log(
        `📝 [EXPLAIN_TERM] Контекст: "${context.substring(0, 100)}..."`
      );
    }
    ...
    console.log(
      `✅ [EXPLAIN_TERM] Объяснение сгенерировано за ${responseTime}ms`
    );
    console.log(
      `📄 [EXPLAIN_TERM] Длина объяснения: ${explanation.length} символов`
    );
