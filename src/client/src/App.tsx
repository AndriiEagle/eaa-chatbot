...
function getDefaultBotMessage(): Message {
  return {
    ...
    content:
      "Hello! I'll help you quickly understand European Accessibility Act requirements. Ask a question or clarify a few details about your company — and I'll guide you on what to do.",
    ts: Date.now(),
    suggestions: [
...
export default function App() {
  ...
  const [isProactiveAgentEnabled, setIsProactiveAgentEnabled] =
    useState<boolean>(() => {
  ...
  useEffect(() => {
    localStorage.setItem(
      ...
      JSON.stringify(isProactiveAgentEnabled)
    );
  }, [isProactiveAgentEnabled]);
    ...
    const fetchWelcome = async () => {
      try {
        const response = await fetch(
          `${API_URL.replace('/ask', '')}/welcome/${userId}`
        );
        ...
        const greetingMsg: Message = {
          ...
          content:
            data.greeting ||
            "Hello! Let's discuss the European Accessibility Act.",
          ts: Date.now(),
        };

        const suggestionsArr: string[] = Array.isArray(data.suggestions)
          ? data.suggestions.slice(0, 3)
          : [];

        const suggestionsMsg: Message | null =
  ...
  const fetchSessions = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL.replace('/ask', '')}/chat/sessions/${userId}`
      );
      if (!response.ok) {
  ...
  const loadSession = async (newSessionId: string) => {
    try {
      ...
      const response = await fetch(
        `${API_URL.replace('/ask', '')}/chat/messages/${newSessionId}`
      );
      if (!response.ok) {
      ...
      logDebug(
        'success',
        `Загружено ${data.length} сообщений из сессии ${newSessionId}`
      );
    } catch (error) {
  ...
  const deleteSession = async (id: string) => {
    try {
      ...
      const response = await fetch(
        `${API_URL.replace('/ask', '')}/chat/sessions/${id}`,
        {
          method: 'DELETE',
        }
      );
  ...
  const sendMessage = async (inputText: string) => {
    ...
    if (suggestionContext.trim()) {
      ...
      console.log(
        '💡 [App] Добавлен контекст подсказки к запросу:',
        suggestionContext
      );
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Server error: ${res.status} - ${errorData.message || 'Unknown error'}`
        );
      }
      ...
      if (contentType.includes('application/json')) {
        ...
        const finalBotMsg = {
          ...
          content:
            data.answer || data.content || data.results || 'Ответ получен',
        };
  ...
  const handleSuggestedQuestion = (question: string) => {
    ...
    if (question.trim().endsWith('-')) {
      ...
      logDebug(
        'info',
        'Шаблон помещен в поле ввода для завершения пользователем'
      );
    } else {
