...
export class SuggestionRepository {
  async getUserFacts(userId: string): Promise<UserFact[]> {
    ...
    try {
      ...
      if (!Array.isArray(facts)) {
        console.warn(
          `⚠️ [REPOSITORY] getUserFacts returned non-array for user ${userId}`
        );
        return [];
      }
      ...
      console.log(
        `✅ [REPOSITORY] Retrieved ${validFacts.length} valid facts for user ${userId}`
      );
      return validFacts;
    } catch (error) {
      console.error(
        `❌ [REPOSITORY] Error fetching user facts for ${userId}:`,
        error
      );
  ...
  async getSessionMessages(sessionId: string): Promise<ChatMessage[]> {
    ...
    try {
      ...
      if (!Array.isArray(messages)) {
        console.warn(
          `⚠️ [REPOSITORY] getSessionMessages returned non-array for session ${sessionId}`
        );
        return [];
      }
      ...
      const validMessages = messages.filter(
        (message: any): message is ChatMessage => {
          return (
            message &&
      ...
      console.log(
        `✅ [REPOSITORY] Retrieved ${validMessagesWithUpdatedAt.length} valid messages for session ${sessionId}`
      );
      return validMessagesWithUpdatedAt;
    } catch (error) {
      console.error(
        `❌ [REPOSITORY] Error fetching session messages for ${sessionId}:`,
        error
      );
      return [];
    }
  ...
  async getUserSessions(userId: string): Promise<ChatSession[]> {
    ...
    try {
      ...
      if (!Array.isArray(sessions)) {
        console.warn(
          `⚠️ [REPOSITORY] getUserSessions returned non-array for user ${userId}`
        );
        return [];
      }
      ...
      const validSessions = sessions.filter(
        (session: any): session is ChatSession => {
          return (
            session &&
      ...
      console.log(
        `✅ [REPOSITORY] Retrieved ${validSessions.length} valid sessions for user ${userId}`
      );
      return validSessions;
    } catch (error) {
      console.error(
        `❌ [REPOSITORY] Error fetching user sessions for ${userId}:`,
        error
      );
      return [];
    }
  ...
  async getFrustrationHistory(userId: string): Promise<FrustrationAnalysis[]> {
    ...
    console.log(
      `😤 [REPOSITORY] Fetching frustration history for user ${userId}`
    );
    ...
    try {
      ...
      if (error) {
        console.error(
          `❌ [REPOSITORY] Supabase error fetching frustration history:`,
          error
        );
        return [];
      }
      ...
      if (!Array.isArray(data)) {
        console.warn(
          `⚠️ [REPOSITORY] Supabase returned non-array for frustration history`
        );
        return [];
      }
      ...
      const validAnalyses = data.filter(
        (analysis: any): analysis is FrustrationAnalysis => {
          return (
            analysis &&
            typeof analysis.user_id === 'string' &&
            typeof analysis.session_id === 'string' &&
            typeof analysis.frustration_level === 'number' &&
            analysis.frustration_level >= 0 &&
            analysis.frustration_level <= 1
          );
        }
      );

      console.log(
        `✅ [REPOSITORY] Retrieved ${validAnalyses.length} valid frustration analyses for user ${userId}`
      );
      return validAnalyses;
    } catch (error) {
      console.error(
        `❌ [REPOSITORY] Error fetching frustration history for ${userId}:`,
        error
      );
      return [];
    }
