...
export function isShortOrSingleQuestion(text: string): boolean {
  ...
  if (questionMarks > 1) {
    console.log('🔍 [DEBUG] Found multiple question marks, consider it multiple question');
    return false;
  }
  ...
  const questionWords = [
    // Russian questions
    'что', 'как', 'где', 'когда', 'почему', 'зачем', 'кто', 'какой', 'какая', 'какое', 'какие', 'чей', 'который',
    // English questions  
    'what', 'how', 'where', 'when', 'why', 'who', 'which', 'whose', 'whom',
    // Descriptive constructions in Russian
    'нужно', 'надо', 'необходимо', 'требуется', 'хочу', 'интересует', 'расскажи', 'объясни', 'помоги', 'подскажи',
    // Descriptive constructions in English
    'need', 'want', 'should', 'must', 'required', 'tell', 'explain', 'help', 'interested', 'provide'
  ];
  ...
  for (const w of questionWords) {
    ...
    if (found > 1) {
      console.log('🔍 [DEBUG] Found more than one keyword, consider it multiple question');
      return false;
    }
  }
  ...
  if (sentenceCount > 1) {
    ...
    console.log('🔍 [DEBUG] More than one sentence, will use additional analysis');
    return false;
  }
...
export async function smartSplitQuestions(input: string): Promise<string[]> {
  ...
  const parts = input.match(/[^?]+\?/g)?.map(q => q.trim()).filter(q => q.length > 2) || [];
  console.log(`🔍 [DEBUG] Split by question marks gave ${parts.length} parts:`, parts);
  ...
  console.log('🔍 [DEBUG] Regular split didn\'t work, use semantic splitting through GPT');
...
export async function semanticSplitQuestions(input: string): Promise<string[]> {
  ...
  try {
    ...
    if (parsedResponse.questions && Array.isArray(parsedResponse.questions)) {
      console.log('🔍 [DEBUG] Successfully extracted questions array from JSON');
      return parsedResponse.questions.map((q: string) => String(q).trim()).filter(Boolean);
    }
    ...
    for (const key in parsedResponse) {
      if (Array.isArray(parsedResponse[key])) {
        ...
        return parsedResponse[key].map((q: string) => String(q).trim()).filter(Boolean);
      }
    }
    ...
    if (Array.isArray(parsedResponse)) {
      ...
      return parsedResponse.map((q: string) => String(q).trim()).filter(Boolean);
    }
