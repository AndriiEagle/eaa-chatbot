import { ChatSummary, ChatMessage, RelevantMessage, UserFact } from './types.js';
...
export class SummaryManager {
  constructor(
    ...
    private findSimilarMessages: (embedding: number[], userId: string, limit?: number) => Promise<RelevantMessage[]>
  ) {}
  ...
  async updateSessionSummary(sessionId: string): Promise<void> {
    try {
      ...
      if (sessionError) {
        console.error(`❌ [MEMORY] Error getting session ${sessionId}:`, sessionError);
        return;
      }
...
Верни результат в следующем формате JSON:
{
  ...
  "business_info": {
    ...
    "digital_presence": "описание digital-присутствия или null"
      ...
      if (error) {
        console.error(`❌ [MEMORY] Error updating session summary ${sessionId}:`, error);
        return;
      }

      console.log(`✅ [MEMORY] Обновлено саммари для сессии ${sessionId}`);
    } catch (e) {
      console.error(`❌ [MEMORY] Exception updating session summary ${sessionId}:`, e);
    }
  }
  ...
  async createContextForRequest(
    ...
    try {
      ...
      try {
        ...
        similarMessages = await this.findSimilarMessages(queryEmbedding, userId, 5);
      } catch (e) {
      ...
      if (userFacts.length > 0) {
        ...
        if (businessFacts.length > 0) {
          ...
          const businessSize = businessFacts.find((f: UserFact) => f.fact_type === 'business_size')?.fact_value;
          ...
          if (businessLocation) context += `- Расположение: ${businessLocation}\n`;
          if (businessSize) context += `- Размер: ${businessSize}\n`;
          if (digitalPresence) context += `- Цифровое присутствие: ${digitalPresence}\n`;
          ...
          } else if (businessType?.toLowerCase().includes('банк') || businessType?.toLowerCase().includes('финанс')) {
          ...
          } else if (businessType?.toLowerCase().includes('магазин') || businessType?.toLowerCase().includes('торгов')) {
          ...
          const otherFacts = userFacts.filter((f: UserFact) => !f.fact_type.startsWith('business_') && f.confidence > 0.7);
