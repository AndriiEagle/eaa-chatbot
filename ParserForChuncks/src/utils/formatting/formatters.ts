...
function preprocessContent(content: any): string {
  ...
  if (Array.isArray(content)) {
    ...
    const allPrimitives = content.every(item => 
...
export function formatSourcesMetadata(
  ...
  console.log(`[FORMATTERS] Первый источник полностью: ${JSON.stringify(list[0])}`);
  ...
  return list.map(chunk => {
    ...
    console.log(`🔎 [FORMATTERS] Источник: id=${chunk.id}, title=${title}, similarity=${chunk.similarity || 0}`);
...
export function formatJsonObjects(text: string | null | undefined): string {
  ...
  console.log(`>>> [FORMATTERS] formatJsonObjects ВХОД: ${typeof text === 'string' ? text.substring(0, 100) + '...' : typeof text}`);
  ...
  if (formattedText.match(listObjectsPattern)) {
    ...
    if (formattedText.toLowerCase().includes('преимуществ') || 
        formattedText.toLowerCase().includes('benefits') ||
        formattedText.toLowerCase().includes('выгод')) {
      
      console.log('🔄 [FORMATTERS] Похоже на список преимуществ доступности, применяю специальное форматирование');
      ...
      formattedText = formattedText.replace(listObjectsPattern, (match) => {
        ...
        const items = Array.from({length: count}, (_, i) => {
          ...
          const benefit = i < benefits.length ? benefits[i] : `Преимущество ${i+1}`;
          return `${i+1}. ${benefit}`;
        }).join('\n');
        return items;
      });
      ...
      formattedText = formattedText.replace(listObjectsPattern, (match) => {
        const count = (match.match(/\d+\./g) || []).length;
        const items = Array.from({length: count}, (_, i) => `${i+1}. Пункт ${i+1}`).join('\n');
        return items;
      });
    }
  }
  ...
  if (listItemObjectPattern.test(formattedText)) {
    console.log('🔄 [FORMATTERS] Обнаружен элемент списка с [object Object], заменяю на описательный текст');
    formattedText = formattedText.replace(listItemObjectPattern, '$1(детали элемента списка)');
  }
  ...
  if (genericObjectPattern.test(formattedText)) {
    console.log('🔄 [FORMATTERS] Обнаружен [object Object], заменяю на описательный текст');
  ...
  if (objectListPattern.test(formattedText)) {
    console.log('🔄 [FORMATTERS] Обнаружен список [object Object], применяю специальное форматирование');
  ...
  console.log(`<<< [FORMATTERS] formatJsonObjects ВЫХОД: ${formattedText.substring(0, 100)}...`);
