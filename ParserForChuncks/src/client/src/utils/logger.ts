 ...
 * Типы сообщений лога
 */
type LogType = 'info' | 'success' | 'warn' | 'error' | 'event' | 'data' | 'time';
 ...
 */
export function logDebug(type: LogType, message: string, data?: any): void {
  if (!DEBUG_LOGS) {
    // Оставляем только важные типы
    if (!['error', 'warn', 'success', 'event', 'info'].includes(type)) return;
  }
