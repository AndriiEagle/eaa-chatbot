  ...
  try {
    ...
    if (simpleQueryResult.is_simple_query && simpleQueryResult.response_text) {
      ...
      const responsePayload = {
        answer: simpleQueryResult.response_text,
        sources: [],
        performance: { embedding_ms: 0, search_ms: 0, generate_ms: 0 },
        session_id: session_id || 'new_session',
        query_id: 'simple-query-' + uuidv4(),
        suggestions: [],
        suggestions_header: '',
      };
      
      // Простые диалоги (приветствия, благодарности) не сохраняем в историю
      // Они не несут важной информации для контекста будущих разговоров
