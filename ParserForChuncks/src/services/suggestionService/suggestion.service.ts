...
export class SuggestionService {
  ...
  constructor(private openai: OpenAI) {
    ...
    console.log(
      '🤖 [SUGGESTION-SERVICE] Initialized with AI-POWERED analyzers'
    );
  }

  async generateRevolutionarySuggestions(
    ...
    try {
      ...
      const [
        ...
        this.businessMaturityAnalyzer.analyze(
          userFacts,
          sessionMessages as any
        ),
        this.conversationStageAnalyzer.analyze(
          sessionMessages as any,
          userFacts as any
        ),
      ]);
      ...
      console.log(
        `   👤 User Persona: ${aiUserPersona.type} (${aiUserPersona.confidence})`
      );
      console.log(
        `   🔄 Behavior Patterns: ${aiBehaviorPatterns.length} detected`
      );
      console.log(
        `   😤 Frustration Level: ${(aiFrustrationProfile.currentLevel * 100).toFixed(1)}%`
      );
      ...
      const predictedNeeds = await this.needsPredictor.predict(
        aiUserPersona,
        businessMaturity,
        conversationStage
      );
      console.log(
        `🔮 [PREDICTION] ${predictedNeeds.length} predicted needs identified`
      );
      ...
      const smartSuggestions =
        await this.generateSmartSuggestions(revolutionaryContext);
      console.log(
        `💡 [SUGGESTIONS] Generated ${smartSuggestions.length} smart suggestions`
      );

      // 🎪 STAGE 11: OPTIMIZE SUGGESTIONS
      const optimizedSuggestions = this.optimizeSuggestionsForUser(
        smartSuggestions,
        revolutionaryContext
      );
      console.log(
        `🎯 [OPTIMIZATION] Optimized to ${optimizedSuggestions.length} top suggestions`
      );
      ...
      return {
        ...
        infoTemplates: [],
        suggestions_header: this.generateDynamicHeader(
          aiUserPersona,
          aiFrustrationProfile,
          conversationStage
        ),
  ...
  private calculateOpportunityScore(
    ...
    const score =
      readinessFactor * 0.4 + frustrationFactor * 0.4 + stageFactor * 0.2;
  ...
  private optimizeSuggestionsForUser(
    ...
    const filtered = suggestions.filter(
      s => s.text.toLowerCase() !== context.currentQuestion.toLowerCase()
    );
    ...
    const scored = filtered.map(s => ({
      ...s,
      complexScore:
        this.calculatePriorityScore(s.priority) +
        s.businessValue * 0.3 +
        context.opportunityScore * 0.3,
    }));

    // Return top 5 suggestions
    return scored.sort((a, b) => b.complexScore - a.complexScore).slice(0, 5);
  }
