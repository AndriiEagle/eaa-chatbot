import { AskRequest, ProcessingResult } from '../../types/common';
import { QuestionProcessingService } from '../questionProcessingService';
import { EmbeddingService } from '../embeddingService';
import { MemoryService } from '../memoryService';
import { SuggestionService } from '../suggestionService/suggestion.service';
import { FrustrationService } from '../frustrationService';
import { openai } from '../openaiService';
import { logger } from '../../utils/logger';
import { createTimer } from '../../utils/metrics/timers.js';
import { chatMemory } from '../../utils/memory/index.js';
import { checkSessionExists, createSessionWithId } from '../../utils/memory/index.js';
  ...
  async processRequest(params: AskRequest): Promise<ProcessingResult> {
    ...
    if (!sessionId) {
      try {
        ...
        console.log(`📝 [ASK] Created new session: ${sessionId} for user: ${user_id}`);
      } catch (error) {
        console.error('❌ [ASK] Failed to create session, using fallback ID:', error);
      ...
      try {
        console.log(`🔍 [ASK] About to call checkSessionExists with: ${sessionId}`);
      ...
      if (!sessionExists) {
        console.log(`⚠️ [ASK] Session ${sessionId} doesn't exist, creating new one...`);
        ...
        try {
          ...
          console.error(`❌ [ASK] Failed to create session with ID ${sessionId}:`, error);
          // Fallback: создать сессию с новым ID
          try {
            const newSessionId = uuidv4();
            await createSessionWithId(newSessionId, user_id, {});
            currentSessionId = newSessionId;
            console.log(`✅ [ASK] Created fallback session: ${currentSessionId}`);
          } catch (fallbackError) {
            console.error(`❌ [ASK] Failed to create fallback session:`, fallbackError);
    ...
    try {
      ...
      // Step 5: Process single question via dedicated flow
        ...
        finalResult = await this.processSingleQuestion(
      ...
      try {
        const notification = await this.frustrationService.analyzeAndHandle(user_id, sessionId, question, finalResult.answer);
        if (notification) {
          // Add escalation notification to the answer
          finalResult.answer = finalResult.answer + `\n\n---\n\n${notification}`;
        }
      } catch (error) {
