...
class AISuggestionsService {
  async generateSuggestions(request: {
    ...
    return {
      suggestions: [
        {
          text: 'What are the basic EAA requirements for my business?',
          category: 'learning_path',
          priority: 8,
          businessValue: 0.9,
          reasoning: 'Basic understanding needed',
        },
        {
          text: 'How to conduct an accessibility audit?',
          category: 'immediate_need',
          priority: 7,
          businessValue: 0.8,
          reasoning: 'Audit is first step',
        },
        {
          ...
          priority: 6,
          businessValue: 0.7,
          reasoning: 'Risk awareness important',
        },
      ],
...
export const generateAISuggestions = async (req: Request, res: Response) => {
  ...
  } = req.body as AISuggestionsRequest;
  ...
  try {
    ...
    res.status(200).json({
      clarificationQuestions: result.suggestions.map(
        (s: SmartSuggestion) => s.text
      ),
      infoTemplates: [],
