...
export const askController = async (
  ...
  console.log(
    `👤 User ID: ${user_id} | 💬 Session ID: ${finalSessionId || 'new'}`
  );
        ...
        const messages = [
          {
            ...
            content:
              'You are a helpful assistant specialized in European Accessibility Act (EAA) compliance.',
          },
          {
            role: 'user' as const,
            content: `Context: ${result.answer}\n\nQuestion: ${question}\n\nProvide a comprehensive answer based on the context provided.`,
          },
        ];
