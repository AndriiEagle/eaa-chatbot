...
class WhisperAdvancedProcessor {
    ...
    mediaRecorder.ondataavailable = async event => {
      if (event.data.size > 0) {
        ...
        if (audioChunks.length >= 3) {
          // Process every 3 chunks
          const combinedBlob = new Blob(audioChunks.slice(-3), {
            type: 'audio/webm',
          });
  ...
  private async processChunksInParallel(
    ...
    return results.sort(
      (a, b) => (a as any).chunkIndex - (b as any).chunkIndex
    );
  }
  ...
  private async assembleTranscription(
    ...
    const averageConfidence =
      chunkResults.reduce((sum, result) => sum + result.confidence, 0) /
      chunkResults.length;
    ...
    for (let channel = 0; channel < numberOfChannels; channel++) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        for (
          ...
          j < Math.min(length, i + windowSize);
          j++
        ) {
  ...
  private async applyVoiceActivityDetection(
    audioBuffer: AudioBuffer
  ): Promise<AudioBuffer> {
    ...
    for (let i = 0; i < voiceFrames.length; i++) {
      if (voiceFrames[i]) {
        for (
          let j = Math.max(0, i - paddingFrames);
          j < Math.min(voiceFrames.length, i + paddingFrames);
          j++
        ) {
    ...
    const voiceBuffer = this.audioContext.createBuffer(
      numberOfChannels,
      newLength,
      sampleRate
    );
  ...
  private normalizeAudio(audioBuffer: AudioBuffer): AudioBuffer {
    ...
    for (let channel = 0; channel < numberOfChannels; channel++) {
      ...
      const normalizationFactor =
        maxAmplitude > 0 ? targetPeak / maxAmplitude : 1;
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (Math.abs(sample) > 0.95) {
          sample =
            Math.sign(sample) *
            (0.95 + 0.05 * Math.tanh((Math.abs(sample) - 0.95) * 10));
        }
  ...
  private async detectSpeechBoundaries(
    audioBuffer: AudioBuffer
  ): Promise<number[]> {
    ...
    for (let i = 0; i < length - frameSize; i += hopSize) {
      ...
      for (let j = i; j < i + frameSize; j++) {
        ...
        if (
          j > i &&
          Math.sign(channelData[j]) !== Math.sign(channelData[j - 1])
        ) {
          zcr++;
        }
      ...
      features.push({
        ...
        zcr: zcr / frameSize,
        spectralCentroid:
          spectralSum > 0 ? spectralWeightedSum / spectralSum : 0,
      });
    }
    ...
    for (let i = 1; i < features.length - 1; i++) {
      ...
      const energyRise =
        (next.energy - current.energy) / (current.energy + 1e-10);

      // Detect spectral changes (topic/speaker changes)
      const spectralChange = Math.abs(
        current.spectralCentroid - prev.spectralCentroid
      );

      const sampleTime = i * hopSize;
      const isValidBoundary =
        sampleTime - boundaries[boundaries.length - 1] >= minBoundaryDistance;
  ...
  private generateContextPrompt(
    ...
    let prompt =
      'This is a discussion about European Accessibility Act (EAA). ';
  ...
  private mergeOverlappingTranscripts(
  ...
  private async detectSpeakers(
    ...
    results.forEach((result, chunkIndex) => {
      ...
      if (transcript.includes('вопрос') || transcript.includes('question')) {
        ...
        transcript.includes('отвечаю') ||
        transcript.includes('ответ') ||
        transcript.includes('answer')
      ) {
        speakerId = 'speaker_responder';
      } else if (
        transcript.includes('добавлю') ||
        transcript.includes('также') ||
        transcript.includes('дополню')
      ) {
