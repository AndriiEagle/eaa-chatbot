...
export function analyzeUserData(
...
): UserDataAnalysisResult {
  console.log(
    '[UserDataAnalysis] Analyzing user data based on facts:',
    userFacts
  );
  ...
  const missingData = requiredFactTypes.filter(
    type => !existingData.includes(type)
  );

  // Filter facts for quick access to the most important ones
  const businessType =
    userFacts.find(fact => fact.type === 'business_type')?.value || '';
  const businessSize =
    userFacts.find(fact => fact.type === 'business_size')?.value || '';
  const customerBase =
    userFacts.find(fact => fact.type === 'customer_base')?.value || '';
  const serviceTypes =
    userFacts.find(fact => fact.type === 'service_types')?.value || '';
  ...
  if (
    !existingData.includes('web_presence') &&
  ...
  if (
    !existingData.includes('service_types') &&
    !existingData.includes('product_types')
  ) {
    specificQuestions.push(
      `What specific ${businessType ? 'services or products do you offer in the ' + businessType + ' sector' : 'services or products do you offer'}?`
    );
  }
  ...
  if (missingData.length > 0) {
    ...
    if (businessType) {
      if (businessType.includes('transport')) {
        aiInstructions +=
          'Especially important to find out: types of transport services, presence of website/app for booking, EU service countries. ';
      } else if (
        businessType.includes('retail') ||
        businessType.includes('commerce')
      ) {
        aiInstructions +=
          'Especially important to find out: product types, presence of e-commerce, EU delivery countries. ';
      } else if (
        businessType.includes('media') ||
        businessType.includes('info')
      ) {
        aiInstructions +=
          'Especially important to find out: types of media content, distribution formats, accessibility on different devices. ';
      }
    }
  }
...
export function formatMissingDataTypes(dataTypes: string[]): string[] {
  ...
  return dataTypes.map(type => {
    switch (type) {
      ...
      case 'service_types':
        return 'types of provided services';
      case 'product_types':
