...
export async function processBusinessInfo(
  params: BusinessInfoParams
): Promise<EnhancedResponse> {
  console.log(
    '🔍 [BUSINESS PROCESS] Обработка бизнес-информации из сообщения пользователя'
  );
  ...
  try {
    ...
    console.log(
      `✅ [BUSINESS PROCESS] Сообщение пользователя сохранено с ID: ${messageId}`
    );
  } catch (error) {
  ...
  try {
    const response = JSON.parse(
      factExtraction.choices[0].message.content || '{}'
    );
  ...
  const businessType = highConfidenceBusinessFacts.find(
    (f: UserFact) => f.fact_type === 'business_type'
  )?.fact_value;
  const businessLocation = highConfidenceBusinessFacts.find(
  ...
  try {
    await chatMemory.saveMessage(params.question, 'user', params.session_id, {
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    console.error('❌ Error saving user message to chat history:', error);
