...
export class AIUserPersonaAnalyzer {
  ...
  async analyze(
    ...
    console.log(
      `🤖 [AI-PERSONA] Starting AI persona analysis with ${userFacts.length} facts and ${sessionMessages.length} messages`
    );

    try {
      // Prepare comprehensive persona data for AI
      const personaData = this.preparePersonaDataForAI(
        userFacts,
        sessionMessages
      );
      ...
      console.log(
        `🤖 [AI-PERSONA] AI identified persona: ${persona.role} (${persona.experienceLevel}) - ${persona.primaryMotivation}`
      );
      return persona;
    } catch (error) {
      console.error(
        '❌ [AI-PERSONA] AI analysis failed, using fallback:',
        error
      );
  ...
  private preparePersonaDataForAI(
    facts: UserFact[],
    ...
    const personaData = {
      businessProfile: {
        ...
        businessType:
          facts.find(f => f.fact_type === 'business_type')?.fact_value ||
          'unknown',
        businessSize:
          facts.find(f => f.fact_type === 'business_size')?.fact_value ||
          'unknown',
        webPresence:
          facts.find(f => f.fact_type === 'web_presence')?.fact_value ||
          'unknown',
        complianceStatus:
          facts.find(f => f.fact_type === 'compliance_status')?.fact_value ||
          'unknown',
      },
      communicationStyle: {
      ...
      professionalIndicators: {
        ...
        problemSolvingApproach:
          this.analyzeProblemSolvingApproach(userMessages),
      },
      learningPatterns: {
...
`;
  ...
  private fallbackPersonaAnalysis(
    facts: UserFact[],
    messages: ChatMessage[]
  ): UserPersona {
    const businessType =
  ...
  private analyzeSupportExpectations(messages: ChatMessage[]): string {
    const highExpectationIndicators = [
      'comprehensive',
      'detailed',
      'complete',
      'thorough',
