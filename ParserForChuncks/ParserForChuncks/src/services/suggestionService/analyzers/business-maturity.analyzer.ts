...
export class BusinessMaturityAnalyzer {
  analyze(
    ...
    const businessType =
      userFacts.find(f => f.fact_type === 'business_type')?.fact_value || '';
    const hasWebsite = userFacts.some(
      f =>
        f.fact_type === 'business_digital_presence' &&
        f.fact_value.includes('website')
    );
    const hasAudit = userFacts.some(
      f => f.fact_type === 'accessibility_audit_done' && f.fact_value === 'true'
    );

    // Determine maturity level based on facts
    if (
      businessType.includes('enterprise') ||
    ...
    if (
      allMessageText.includes('urgent') ||
      allMessageText.includes('deadline')
    ) {
      timeConstraints = 'urgent';
    } else if (
      allMessageText.includes('soon') ||
      allMessageText.includes('quickly')
    ) {
