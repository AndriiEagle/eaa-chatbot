...
export class BehaviorPatternAnalyzer {
  async analyze(
    ...
    const communicationPatterns =
      this.analyzeCommunicationStyle(sessionMessages);
    ...
    const problemSolvingPatterns =
      this.analyzeProblemSolvingBehavior(sessionMessages);
  ...
  private analyzeCommunicationStyle(
    ...
    const complexQuestions = userMessages.filter(
      m => m.content.includes('?') && m.content.split('?').length > 2
    );
    ...
    if (complexQuestions.length > userMessages.length * 0.3) {
      patterns.push({
        ...
        context:
          'Uses diverse vocabulary, likely handles complex concepts well',
        significance: 0.8,
      });
    }
  ...
  private analyzeLearningProgression(
    ...
    const questionComplexity = userMessages.map((message, index) => {
      ...
      if (content.includes('what is') || content.includes('how to'))
        complexity += 1;
      ...
      if (content.includes('implement') || content.includes('integrate'))
        complexity += 3;

      // Expert question indicators
      if (content.includes('optimize') || content.includes('best practice'))
        complexity += 4;
    ...
    const earlyComplexity = questionComplexity
      .slice(0, Math.ceil(questionComplexity.length / 3))
      .reduce((sum, q) => sum + q.complexity, 0);
    const lateComplexity = questionComplexity
      .slice(-Math.ceil(questionComplexity.length / 3))
      .reduce((sum, q) => sum + q.complexity, 0);
  ...
  private analyzeEngagementPatterns(
    ...
    const avgMessagesPerSession =
      messagesPerSession.reduce((sum, s) => sum + s.messageCount, 0) /
      messagesPerSession.length;
  ...
  private analyzeProblemSolvingBehavior(
    messages: ChatMessage[]
  ): BehaviorPattern[] {
    const patterns: BehaviorPattern[] = [];
    const userMessages = messages.filter(m => m.role === 'user');
  ...
  private getProblemSolvingRecommendation(approach: string): string {
    const recommendations: Record<string, string> = {
      systematic:
        'Provide structured, step-by-step guides with clear progression',
    ...
    return (
      recommendations[approach] || 'Adapt response style to user preferences'
    );
  }
}
