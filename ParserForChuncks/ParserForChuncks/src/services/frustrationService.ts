...
export class FrustrationService {
  ...
  async analyzeAndHandle(
    ...
    try {
      ...
      console.log(
        `📚 [FrustrationAgent] Loading context for session ${sessionId}`
      );
      const sessionMessages = await chatMemory.getSessionMessages(sessionId);
      console.log(
        `📚 [FrustrationAgent] Loaded ${sessionMessages.length} recent messages`
      );

      // Убеждаемся, что контекст не пустой
      if (sessionMessages.length === 0) {
        console.log(
          `⚠️ [FrustrationAgent] No recent messages found, this might be first message in session`
        );
      }

      const contextSummary = sessionMessages
...
А пока я продолжу отвечать на ваши вопросы!
      ...
      logger.error('Error in frustration escalation', {
        error,
        userId,
        sessionId,
      });
