...
class WhisperAdvancedProcessor {
    constructor(openaiClient, monitor) {
     ...
     */
    async processLongAudio(audioBlob, options = {}, onProgress) {
        ...
        try {
            ...
            const transcriptionResults = await this.processChunksInParallel(chunks, options, (chunkProgress) => {
                onProgress?.({
                    stage: 'transcription',
            ...
            const finalResult = await this.assembleTranscription(transcriptionResults, preprocessedAudio, options);
            onProgress?.({
                stage: 'postprocessing',
                progress: 100
            });
            ...
            this.monitor.recordError('voice_processing', error);
     ...
     */
    async processRealTime(audioStream, options = {}, onTranscript) {
     ...
     */
    async preprocessAudio(audioBlob, options) {
     ...
     */
    async intelligentChunking(audioBuffer, options) {
        ...
        while (currentTime < totalDuration) {
            ...
            if (speechBoundaries.length > 0) {
                const nearbyBoundary = speechBoundaries.find(boundary => Math.abs(boundary - chunkEndTime) < 3);
                if (nearbyBoundary) {
            ...
            const chunkBuffer = this.extractAudioSegment(audioBuffer, chunkStartSample, chunkEndSample);
     ...
     */
    async processChunksInParallel(chunks, options, onProgress) {
     ...
     */
    async processShortAudio(audioBlob, options) {
     ...
     */
    async assembleTranscription(chunkResults, originalAudio, options) {
        ...
        const averageConfidence = chunkResults.reduce((sum, result) => sum + result.confidence, 0) / chunkResults.length;
        // Extract metadata
    ...
    async applyVoiceActivityDetection(audioBuffer) {
    ...
    async detectSpeechBoundaries(audioBuffer) {
        ...
        for (let i = 1; i < features.length - 1; i++) {
            ...
            if (isValidBoundary && ((energyDrop > 0.5 && current.energy < energyThreshold) || // Silence gap
    ...
    extractAudioSegment(audioBuffer, startSample, endSample) {
    ...
    generateContextPrompt(chunk, options) {
    ...
    parseWhisperResponse(response, audioBlob) {
    ...
    mergeOverlappingTranscripts(results) {
    ...
    estimateNoiseLevel(audioBuffer) {
    ...
    detectDominantLanguage(results) {
...
export { WhisperAdvancedProcessor };
